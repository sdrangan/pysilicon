"""
CLI entrypoint: build or refresh the OpenAI vector store for waveflow examples.

Usage
-----
As a console script (after ``pip install waveflow``)::

    waveflow-build-example-rag

Or directly::

    python -m waveflow.mcp.cli_build_example_rag

What it does
------------
1. Enumerates text files under the ``waveflow.mcp.corpus`` package.
...
_EXAMPLES_PACKAGE = "waveflow.mcp.corpus"
_VECTOR_STORE_NAME = "waveflow-examples"
_VECTOR_STORE_ENV = "WAVEFLOW_EXAMPLES_VECTOR_STORE_ID"
_OPENAI_SUPPORTED_SUFFIXES = frozenset({".py", ".cpp", ".c", ".h", ".md"})
_PATH_ESCAPE = "__waveflow_path__"
...
def _generate_catalog(files: list[tuple[str, str]]) -> str:
    ...
    lines = [
        "# waveflow examples catalog",
        "",
        "This catalog describes the packaged waveflow MCP corpus.",
...
def build_example_rag(*, verbose: bool = True) -> str:
    """Build (or rebuild) the OpenAI vector store for waveflow examples.
    ...
    if verbose:
        _print_env_var_instructions("WAVEFLOW_EXAMPLES_VECTOR_STORE_ID", vs_id)
...
def main() -> None:
    """Console-script entry point for ``waveflow-build-example-rag``."""
