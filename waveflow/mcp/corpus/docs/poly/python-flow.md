...
The Python side of the example lives in [examples/poly/poly_demo.py](https://github.com/sdrangan/waveflow/blob/main/examples/poly/poly_demo.py). It follows the same structure as the regression in [tests/hw/test_dataschema_poly.py](https://github.com/sdrangan/waveflow/blob/main/tests/hw/test_dataschema_poly.py): define schemas, build test inputs, run a golden model, emit generated headers, and write binary vectors.

## Step 1: Define the schemas

The first task is to define the data structures that represent the accelerator inputs and outputs. In Waveflow, these structures are specified using `DataSchema` classes. The script [examples/poly/poly_demo.py](https://github.com/sdrangan/waveflow/blob/main/examples/poly/poly_demo.py) defines the following schema classes:
...
Waveflow can auto-generate an include file such as `poly_cmd_hdr.h` for each `DataSchema` class. Each generated header defines a Vitis HLS C++ data structure together with serialization and deserialization support.
...
Activate the `waveflow` virtual environment.
