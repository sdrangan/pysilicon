...
import waveflow.mcp.cli_build_example_rag as rag_cli
...
def test_build_example_rag_deletes_previous_store_from_env(monkeypatch):
    ...
    monkeypatch.setenv("WAVEFLOW_EXAMPLES_VECTOR_STORE_ID", "vs_old")
...
def test_print_env_var_instructions_shows_unix_and_powershell_commands(capsys):
    rag_cli._print_env_var_instructions("WAVEFLOW_EXAMPLES_VECTOR_STORE_ID", "vs_test_123")

    captured = capsys.readouterr()
    output = captured.out

    assert "Unix/Linux/macOS (current shell):" in output
    assert "export WAVEFLOW_EXAMPLES_VECTOR_STORE_ID=vs_test_123" in output
    assert "PowerShell (current session):" in output
    assert '$env:WAVEFLOW_EXAMPLES_VECTOR_STORE_ID = "vs_test_123"' in output
    assert "PowerShell (persist for future sessions):" in output
    assert 'setx WAVEFLOW_EXAMPLES_VECTOR_STORE_ID "vs_test_123"' in output


def test_build_example_rag_skips_unsupported_corpus_extensions(monkeypatch):
    ...
    class FakeVectorStoresApi:
        ...
        def retrieve(self, vector_store_id):
            ...
            return SimpleNamespace(
                id="vs_new",
                status="completed",
                file_counts=SimpleNamespace(completed=len(uploaded_names), failed=0),
            )
    ...
    monkeypatch.setattr(
        rag_cli,
        "_enumerate_example_files",
        lambda: [
            ("examples/poly/poly.py", "x = 1"),
            ("examples/poly/poly.hpp", "header"),
            ("examples/poly/run.tcl", "script"),
            ("docs/poly/index.md", "# doc"),
        ],
    )
    monkeypatch.setattr(rag_cli, "_generate_catalog", lambda files: "catalog")
    monkeypatch.setattr(rag_cli, "_wait_for_vector_store", lambda client, vs_id: None)
    ...
    assert uploaded_names == [
        "_CATALOG.md",
        "examples__waveflow_path__poly__waveflow_path__poly.py",
        "docs__waveflow_path__poly__waveflow_path__index.md",
    ]


def test_encode_decode_upload_filename_round_trip():
    original = "examples/conv2d/conv2d.py"
    encoded = rag_cli.encode_upload_filename(original)

    assert encoded == "examples__waveflow_path__conv2d__waveflow_path__conv2d.py"
    assert rag_cli.decode_upload_filename(encoded) == original