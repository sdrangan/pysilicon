"""
Tests for waveflow MCP tools: schema_examples, registry, and server.
"""
...
from waveflow.mcp.example_rag import get_example_file, search_schema_examples
...
from waveflow.mcp.schema_tools import get_schema_draft_plan, validate_schema
...
def test_registry_has_expected_tools():
    ...
    assert "waveflow_list_schema_examples" not in names
    assert "waveflow_get_schema_example" not in names
    assert "waveflow_get_schema_draft_plan" in names
    assert "waveflow_validate_schema" in names
    assert "waveflow_get_components" in names
    assert "waveflow_search_examples" in names
    assert "waveflow_search_schema_examples" in names
    assert "waveflow_get_example_file" not in names
...
def test_get_schema_draft_plan_first_step_uses_workspace_root_when_provided():
    ...
    assert "waveflow_search_examples" in result["steps"][0]["instructions"]
...
def test_get_schema_draft_plan_first_step_uses_only_example_tools_without_workspace_root():
    ...
    assert "waveflow_search_examples" in instructions
...
def test_registry_dispatch_get_example_file_raises_value_error():
    with pytest.raises(ValueError, match="Unknown tool name"):
        REGISTRY.dispatch("waveflow_get_example_file", {"path": "hist.py"})
