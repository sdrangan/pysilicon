"""
Headless LLM harness for waveflow MCP tools.

This module simulates a user interacting with the waveflow MCP tools
...
(``waveflow.mcp.registry``) so tool definitions are never duplicated.

Usage
-----
Run interactively (requires ``OPENAI_API_KEY`` in the environment)::

    python -m waveflow.mcp.headless
...
Or call from Python::
    ...
    from waveflow.mcp.headless import run_session
...
from waveflow.mcp.file_tools import _resolve_safe, make_file_tools
...
SYSTEM_PROMPT = """\
You are helping a hardware engineer author waveflow code.

You are a new user with no prior knowledge of the waveflow functions.
...
starting with the waveflow vocabulary glossary and semantic example search.

waveflow schemas define the shape of streaming data exchanged with hardware
...
Your goal is to help the user understand how to write waveflow code that meets
...
def _build_tool_runtime(
    *,
    ...
    def dispatch_tool(name: str, arguments: dict[str, Any]) -> Any:
        ...
        if name == "waveflow_validate_schema":
