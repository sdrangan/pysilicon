"""
OpenAI vector-store backed search over the packaged waveflow examples corpus.

Environment variables
---------------------
WAVEFLOW_EXAMPLES_VECTOR_STORE_ID
    The OpenAI vector store ID created by ``waveflow-build-example-rag``.
...
from waveflow.mcp.cli_build_example_rag import decode_upload_filename

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

VECTOR_STORE_ENV = "WAVEFLOW_EXAMPLES_VECTOR_STORE_ID"
...
def search_schema_examples(
    ...
    When ``WAVEFLOW_EXAMPLES_VECTOR_STORE_ID`` is not set this function
    ...
    keywords:
        Optional list of waveflow vocabulary keywords (e.g. ``["DataList",
    ...
    if not vector_store_id:
        return {
            "summary": (
                ...
                "returned by `waveflow-build-example-rag`, then retry."
            ),
            "normalized_query": normalized_query,
            "matches": [],
            "error": (
                f"Environment variable {VECTOR_STORE_ENV!r} is missing or empty. "
                "Run `waveflow-build-example-rag` to create the vector store and "
