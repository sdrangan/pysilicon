...
def _write_and_read_words(schema_obj, path: Path, **kwargs) -> np.ndarray:
    schema_obj.write_uint32_file(path, **kwargs)
...
def test_poly_notebook_flow_generates_headers_vectors_and_expected_outputs(tmp_path: Path):
    ...
    expected_y = np.polynomial.polynomial.polyval(samp_in.val, coeffs.val)
    assert int(resp_hdr.tx_id) == 42
    assert int(resp_ftr.nsamp_read) == nsamp
    assert resp_ftr.error is PolyError.NO_ERROR
    assert np.allclose(samp_out.val, expected_y, rtol=1e-6, atol=1e-6)

    data_dir = tmp_path / "data"
    data_dir.mkdir(parents=True, exist_ok=True)

    cmd_words = _write_and_read_words(cmd_hdr, data_dir / "cmd_hdr_data.bin")
    samp_in_words = _write_and_read_words(samp_in, data_dir / "samp_in_data.bin", nwrite=cmd_hdr.nsamp)
    resp_hdr_words = _write_and_read_words(resp_hdr, data_dir / "resp_hdr_data.bin")
    samp_out_words = _write_and_read_words(samp_out, data_dir / "samp_out_data.bin", nwrite=cmd_hdr.nsamp)
    resp_ftr_words = _write_and_read_words(resp_ftr, data_dir / "resp_ftr_data.bin")

    assert np.array_equal(cmd_words, np.asarray(cmd_hdr.serialize(word_bw=32), dtype="<u4"))
    assert np.array_equal(samp_in_words, _float32_words(samp_in.val))
    assert np.array_equal(resp_hdr_words, np.asarray(resp_hdr.serialize(word_bw=32), dtype="<u4"))
    assert np.array_equal(samp_out_words, _float32_words(samp_out.val))
...
def test_poly_notebook_flow_runs_vitis_csim(tmp_path: Path):
    ...
    assert np.allclose(got_samp_out, expected_samp_out.val[: got_samp_out.size], rtol=1e-6, atol=1e-6)