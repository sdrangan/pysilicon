#ifndef RNORM_ARRAY_H
#define RNORM_ARRAY_H
...
public:
    ...
    static constexpr int bitwidth = 1024;

    static ap_uint<bitwidth> pack_to_uint(const RnormArray& data) {
        ap_uint<bitwidth> res = 0;
        int bitpos = 0;
        for (int i0 = 0; i0 < 32; ++i0) {
            res.range(bitpos + 32 - 1, bitpos) = streamutils::float_to_uint(data.rnorm_elem[i0]);
            bitpos += 32;
        }
        return res;
    }

    static RnormArray unpack_from_uint(const ap_uint<bitwidth>& packed) {
        RnormArray data;
        int bitpos = 0;
        for (int i0 = 0; i0 < 32; ++i0) {
            data.rnorm_elem[i0] = streamutils::uint_to_float((uint32_t)(packed.range(bitpos + 32 - 1, bitpos)));
            bitpos += 32;
        }
        return data;
    }
    ...
    void write_array(ap_uint<word_bw> x[], int n0=1) const {
        if constexpr (word_bw == 32) {
            const int n0_eff = (n0 < 0) ? 0 : ((n0 > 32) ? 32 : n0);
            int out_idx = 0;
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                x[out_idx++] = streamutils::float_to_uint(this->rnorm_elem[i0]);
            }
        }
        else if constexpr (word_bw == 64) {
            const int n0_eff = (n0 < 0) ? 0 : ((n0 > 32) ? 32 : n0);
            ...
            for (int i = 0; i < n0_eff; i += 2) {
                ...
                ap_uint<64> w = 0;
                if (i + 0 < n0_eff) {
                    w.range(31, 0) = streamutils::float_to_uint(this->rnorm_elem[i + 0]);
                }
                if (i + 1 < n0_eff) {
                    w.range(63, 32) = streamutils::float_to_uint(this->rnorm_elem[i + 1]);
                }
                x[out_idx++] = w;
            }
        }
    ...
    void write_stream(hls::stream<ap_uint<word_bw>> &s, int n0=1) const {
        if constexpr (word_bw == 32) {
            ...
            ap_uint<32> w = 0;
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                w = streamutils::float_to_uint(this->rnorm_elem[i0]);
                s.write(w);
                out_idx++;
            }
        }
    ...
    void read_array(const ap_uint<word_bw> x[], int n0=1) {
        if constexpr (word_bw == 32) {
            ...
            int in_idx = 0;
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                this->rnorm_elem[i0] = streamutils::uint_to_float((uint32_t)(x[in_idx]));
                in_idx++;
            }
        }
        else if constexpr (word_bw == 64) {
            ...
            for (int i = 0; i < n0_eff; i += 2) {
                ...
                if (i + 0 < n0_eff) {
                    this->rnorm_elem[i + 0] = streamutils::uint_to_float((uint32_t)(w.range(31, 0)));
                }
                if (i + 1 < n0_eff) {
                    this->rnorm_elem[i + 1] = streamutils::uint_to_float((uint32_t)(w.range(63, 32)));
                }
            }
        }
        else {
    ...
    void read_stream(hls::stream<ap_uint<word_bw>> &s, int n0=1) {
        if constexpr (word_bw == 32) {
            ...
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                w = s.read();
                this->rnorm_elem[i0] = streamutils::uint_to_float((uint32_t)(w));
                in_idx++;
            }
        }
        else if constexpr (word_bw == 64) {
    ...
    void load_json(const std::string& json_text, size_t& pos) {
        ...
        this->rnorm_elem[i0] = static_cast<float>(streamutils::json_parse_number(json_text, pos));
        }
        streamutils::json_expect_char(json_text, pos, ']');
    }
