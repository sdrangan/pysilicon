#ifndef POLY_RESP_HEADER_H
#define POLY_RESP_HEADER_H
...
public:
    ...
    static ap_uint<bitwidth> pack_to_uint(const PolyRespHeader& data) {
        ap_uint<bitwidth> res = 0;
        res.range(15, 0) = data.txn_id;
        return res;
    }

    static PolyRespHeader unpack_from_uint(const ap_uint<bitwidth>& packed) {
        PolyRespHeader data;
