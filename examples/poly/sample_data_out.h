#ifndef SAMPLE_DATA_OUT_H
#define SAMPLE_DATA_OUT_H
...
public:
    ...
    template<int word_bw>
    static int nwords_len(int n0=1) {
        if constexpr (word_bw == 32) {
            const int n0_eff = (n0 < 0) ? 0 : ((n0 > 128) ? 128 : n0);
            const int n_total = n0_eff;
            return (n_total + 1 - 1) / 1;
        }
        else if constexpr (word_bw == 64) {
            const int n0_eff = (n0 < 0) ? 0 : ((n0 > 128) ? 128 : n0);
            const int n_total = n0_eff;
            return (n_total + 2 - 1) / 2;
        }
        else {
            static_assert(word_bw > 0, "Unsupported word_bw for nwords_len");
            return 0;
        }
    }

    static ap_uint<bitwidth> pack_to_uint(const SampleDataOut& data) {
        ap_uint<bitwidth> res = 0;
        int bitpos = 0;
        for (int i0 = 0; i0 < 128; ++i0) {
            res.range(bitpos + 32 - 1, bitpos) = streamutils::float_to_uint(data.y[i0]);
            bitpos += 32;
        }
        return res;
    }

    static SampleDataOut unpack_from_uint(const ap_uint<bitwidth>& packed) {
        SampleDataOut data;
        int bitpos = 0;
        for (int i0 = 0; i0 < 128; ++i0) {
            data.y[i0] = streamutils::uint_to_float((uint32_t)(packed.range(bitpos + 32 - 1, bitpos)));
            bitpos += 32;
        }
        return data;
    }
    ...
    template<int word_bw>
    static void read_axi4_stream_elem(hls::stream<hls::axis<ap_uint<word_bw>, 0, 0, 0>>& s, float out[pf<word_bw>()], int n = pf<word_bw>()) {
        #pragma HLS INLINE
        if constexpr (word_bw == 32) {
            if (n > 0) {
    ...
    template<int word_bw>
    static void write_axi4_stream_elem(hls::stream<hls::axis<ap_uint<word_bw>, 0, 0, 0>>& s, const float in[pf<word_bw>()], bool tlast = false, int n = pf<word_bw>()) {
        #pragma HLS INLINE
        if constexpr (word_bw == 32) {
            if (n > 0) {
                ap_uint<32> w = streamutils::float_to_uint(in[0]);
    ...
    void write_array(ap_uint<word_bw> x[], int n0=1) const {
        if constexpr (word_bw == 32) {
            ...
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                x[out_idx++] = streamutils::float_to_uint(this->y[i0]);
            }
        }
        else if constexpr (word_bw == 64) {
            const int n0_eff = (n0 < 0) ? 0 : ((n0 > 128) ? 128 : n0);
            ...
            for (int i = 0; i < n0_eff; i += 2) {
                ...
                ap_uint<64> w = 0;
                if (i + 0 < n0_eff) {
                    w.range(31, 0) = streamutils::float_to_uint(this->y[i + 0]);
                }
                if (i + 1 < n0_eff) {
                    w.range(63, 32) = streamutils::float_to_uint(this->y[i + 1]);
                }
                x[out_idx++] = w;
            }
        }
    ...
    void write_stream(hls::stream<ap_uint<word_bw>> &s, int n0=1) const {
        if constexpr (word_bw == 32) {
            ...
            ap_uint<32> w = 0;
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                w = streamutils::float_to_uint(this->y[i0]);
                s.write(w);
                out_idx++;
            }
        }
    ...
    void read_array(const ap_uint<word_bw> x[], int n0=1) {
        if constexpr (word_bw == 32) {
            ...
            int in_idx = 0;
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                this->y[i0] = streamutils::uint_to_float((uint32_t)(x[in_idx]));
                in_idx++;
            }
        }
        else if constexpr (word_bw == 64) {
            ...
            for (int i = 0; i < n0_eff; i += 2) {
                ...
                if (i + 0 < n0_eff) {
                    this->y[i + 0] = streamutils::uint_to_float((uint32_t)(w.range(31, 0)));
                }
                if (i + 1 < n0_eff) {
                    this->y[i + 1] = streamutils::uint_to_float((uint32_t)(w.range(63, 32)));
                }
            }
        }
        else {
    ...
    void read_stream(hls::stream<ap_uint<word_bw>> &s, int n0=1) {
        if constexpr (word_bw == 32) {
            ...
            for (int i0 = 0; i0 < n0_eff; ++i0) {
                w = s.read();
                this->y[i0] = streamutils::uint_to_float((uint32_t)(w));
                in_idx++;
            }
        }
        else if constexpr (word_bw == 64) {
    ...
    void dump_json(std::ostream& os, int indent = 2, int level = 0) const {
        ...
        for (int i0 = 0; i0 < 128; ++i0) {
        if (i0 > 0) { os << ","; }
        os << streamutils::float_to_uint(this->y[i0].y);
        }
        os << "]";
    }
    ...
    void load_json(const std::string& json_text, size_t& pos) {
        ...
        throw std::runtime_error("Unsupported array element type for JSON load.");
        }
        streamutils::json_expect_char(json_text, pos, ']');
    }
