...
public:
    ...
    static ap_uint<bitwidth> pack_to_uint(const SampleIn& data) {
    ...
    static SampleIn unpack_from_uint(const ap_uint<bitwidth>& packed) {
        SampleIn data;
        int bitpos = 0;
        for (int i0 = 0; i0 < 128; ++i0) {
    ...
    void load_json(const std::string& json_text, size_t& pos) {
        ...
        this->x[i0] = static_cast<float>(streamutils::json_parse_number(json_text, pos));
        }
        streamutils::json_expect_char(json_text, pos, ']');
    }
