...
The `read_array` and `write_array` helpers handle packing/unpacking between the element type (e.g., `float`, `ap_uint<32>`) and the `mem_word_t` words of the AXI port. They are generated by `ArrayUtilsStep` on the Python side and placed in the `include/` directory.
