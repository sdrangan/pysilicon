...
async function getPythonPath(): Promise<string> {
  const waveflowConfig = vscode.workspace.getConfiguration('waveflow');
  const configured = waveflowConfig.get<string>('pythonPath', '')?.trim() ?? '';

  if (!configured) {
    throw new Error(
      'Waveflow requires waveflow.pythonPath to be set. Run "Waveflow: Select Python Interpreter" or set it in Settings.'
    );
  }

  if (!isExplicitPath(configured)) {
    throw new Error(
      `Waveflow pythonPath must be an explicit interpreter path. Received: ${configured}`
    );
  }

  if (!existsSync(configured)) {
    throw new Error(`Waveflow pythonPath does not exist: ${configured}`);
  }
...
async function pickAndStorePythonPath(): Promise<string | undefined> {
  ...
  const config = vscode.workspace.getConfiguration('waveflow');
  const target = vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0
    ? vscode.ConfigurationTarget.Workspace
    : vscode.ConfigurationTarget.Global;

  await config.update('pythonPath', selectedPath, target);
  void vscode.window.showInformationMessage(`Waveflow pythonPath set to: ${selectedPath}`);
  return selectedPath;
}

export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    'waveflow.addHelloMethod',
  ...
  const selectPythonPathDisposable = vscode.commands.registerCommand(
    'waveflow.selectPythonPath',
    async () => {
      try {
        await pickAndStorePythonPath();
      } catch (error) {
        void vscode.window.showErrorMessage(`Failed to set Waveflow pythonPath: ${String(error)}`);
      }
    }
  );
  ...
  return new Promise((resolve, reject) => {
    const proc = execFile(
      ...
      [
        ...
        `
import sys
from waveflow.dev.create_hello import create_hello
